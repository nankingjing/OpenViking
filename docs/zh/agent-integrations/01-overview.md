...
| **Pi coding agent** | [Pi Coding Agent 扩展](./10-pi-coding-agent.md) — 原生 TypeScript extension，支持自动召回、自动捕获与上下文接管 |
...
- **基于 hooks / 原生插件**（Claude Code、Codex、Pi、OpenClaw、Hermes Agent、AstrBot）：在运行时生命周期事件（每次 prompt、每轮结束、session 起止、compact、subagent 派生等）中驱动召回与捕获。模型不需要"记得调用"。Pi 还支持上下文接管：已提交历史会在模型视角中替换为 OpenViking archive overview 和最近 live turns。
