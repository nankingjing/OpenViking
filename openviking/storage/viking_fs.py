...
class VikingFS:
    ...
    async def _drop_stale_retrieval_hits(
        ...
        stale_uris = {vector_uri for (_, vector_uri), live in checked.items() if not live}
        if vector_store and stale_uris:
            try:
                await vector_store.delete_uris(ctx, sorted(stale_uris))
            except Exception as exc:
