...
const WRITE_OR_EFFECT_RE = /\b(write|edit|modify|delete|remove|migrate|deploy|release|publish|configure|patch)\b|写|改|修改|删除|迁移|部署|发布|配置|打补丁/i;
const EXECUTION_RE = /\b(fix|debug|test|build|run|implement|refactor|integrate|repair|troubleshoot)\b|修复|调试|测试|构建|运行|实现|重构|对接|排查/i;
const FAILURE_RE = /\b(error|exception|traceback|failed|failure|retry|exit code|test failed)\b|报错|异常|失败|重试|挂了|不通过/i;
const ENGINEERING_OBJECT_RE = /(?:^|\s)(?:[\w.-]+\/[\w./-]+|[\w./-]+\.(?:ts|tsx|js|jsx|py|go|rs|java|md|json|ya?ml|toml|sh|sql))\b|`[^`]+`|\b(?:repo|workspace|plugin|service|component|hook|api|tool|package|module)\b|仓库|工作区|插件|服务|组件|接口|工具|模块|文件/i;
const EXPERIENCE_INTENT_RE = /经验|踩坑|最佳实践|不要再|按之前|avoid|best practice|lesson|pitfall/i;
const QUESTION_ONLY_RE = /^(?:什么是|是什么|区别|解释|讲讲|怎么看|为什么|如何理解|where is|what is|explain|difference between)\b|[?？]$/i;
...
function formatMemoryLine(item, content, options) {
...
function toTraceResults(items, resourceType) {
...
function boundTraceQuery(query, maxChars) {
    if (query.length <= maxChars) {
        return { query };
    }
    return { query: query.slice(0, maxChars), queryTruncated: true };
}
function runtimeFlag(runtimeContext, key) {
    return runtimeContext && typeof runtimeContext === "object"
        ? runtimeContext[key]
        : undefined;
}
export function isCronSession(sessionKey, runtimeContext) {
    return Boolean(sessionKey?.includes(":cron:") ||
...
export function shouldRecallAgentExperience(input) {
    ...
    let score = 0;
    if (WRITE_OR_EFFECT_RE.test(text))
        score += 3;
    if (EXECUTION_RE.test(text))
        score += 2;
    if (FAILURE_RE.test(text))
        score += 2;
    if (ENGINEERING_OBJECT_RE.test(text))
        score += 2;
    if (EXPERIENCE_INTENT_RE.test(text))
        score += 1;
...
export async function buildAutoRecallContext(params) {
    ...
    return withTimeout((async () => {
        ...
        const traceSearches = searchPlan.skipped.map((skipped) => ({
        ...
        if (memoryLines.length === 0) {
            verbose?.(`openviking: skipping auto-recall injection; no complete memories fit maxInjectedChars=${maxInjectedChars}`);
            await recordTrace([], 0, 0);
            return { memoryCount: 0, estimatedTokens: 0 };
        }
        const block = buildRecallContextBlock(memoryLines);
        verbose?.(`openviking: injecting ${memoryLines.length} memories (${block.length} chars, ~${estimatedTokens} tokens, maxInjectedChars=${maxInjectedChars})`);
