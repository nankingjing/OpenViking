 ...
 */
async function pollPhase2ExtractionOutcome(client, taskId, agentId, logger, sessionLabel) {
...
export async function commitOpenVikingSession({ sessionId, sessionKey, getClient, logger, resolveAgentId, rememberSessionAgentId, isBypassedSession, }) {
...
export async function afterTurnOpenVikingSession({ sessionId, sessionKey, messages: rawMessages, prePromptMessageCount, isHeartbeat, runtimeContext, cfg, getClient, logger, resolveAgentId, rememberSessionAgentId, isBypassedSession, diag, }) {
    ...
    try {
        ...
        for (const msg of extractedMessages) {
            ...
            if (ovParts.length > 0) {
                await client.addSessionMessage(ovSessionId, msg.role, ovParts, agentId, createdAt, msg.role === "user" ? senderRoleId : undefined);
            }
        }
