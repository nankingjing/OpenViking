...
const METADATA_JSON_KEY_RE = /"(session|sessionid|sessionkey|conversationid|channel|sender|userid|agentid|timestamp|timezone)"\s*:/gi;
...
const QUESTION_CUE_RE = /[?？]|\b(?:what|when|where|who|why|how|which|can|could|would|did|does|is|are)\b|^(?:请问|能否|可否|怎么|如何|什么时候|谁|什么|哪|是否)/i;
function resolveCaptureMinLength(text) {
...
export function sanitizeUserTextForCapture(text) {
    // 过滤 HEARTBEAT 健康检查消息
...
export function stripOpenVikingContextInjection(text) {
    return text
        .replace(OPENVIKING_CONTEXT_BLOCK_RE, " ")
        .replace(RELEVANT_MEMORIES_BLOCK_RE, " ")
        .replace(/\s+/g, " ")
...
export function looksLikeQuestionOnlyText(text) {
...
export function matchesSessionPattern(sessionRef, patterns) {
...
export function shouldBypassSession(params, patterns) {
...
function normalizeCaptureDedupeText(text) {
    return normalizeDedupeText(text).replace(/[\p{P}\p{S}]+/gu, " ").replace(/\s+/g, " ").trim();
}
export function pickRecentUniqueTexts(texts, limit) {
...
export function getCaptureDecision(text, mode, captureMaxLength) {
...
export function extractTextsFromUserMessages(messages) {
    const texts = [];
...
export function extractNewTurnMessages(messages, startIndex) {
            ...
            if (Array.isArray(content)) {
                for (const block of content) {
                    ...
                    if (b?.type === "toolCall" || b?.type === "toolUse" || b?.type === "tool_call") {
                        const id = b.id || b.toolUseId || b.toolCallId;
    ...
    for (let i = startIndex; i < messages.length; i++) {
        ...
        if (role === "toolResult") {
            ...
            const toolCallId = msg.toolCallId || msg.toolUseId || msg.tool_call_id;
        ...
        if (text) {
            ...
            if (cleanedText) {
                ...
                const ovRole = role === "assistant" ? "assistant" : "user";
                result.push({
                    role: ovRole,
                    parts: [{
                            type: "text",
 ...
 */
export function extractNewTurnTexts(messages, startIndex) {
